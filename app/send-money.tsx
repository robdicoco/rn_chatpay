...
export default function SendMoneyScreen() {
  ...
  const [selectedContact, setSelectedContact] = useState(
    receiverId ? contacts.find(c => c.id === receiverId) : null
  );
  ...
  const handleSendMoney = async () => {
    ...
    try {
      ...
      sendMessage(
        `I've sent you $${amount}${note ? ` for ${note}` : ''}`,
        selectedContact.id,
        {
      ...
      Alert.alert(
        'Success',
        `You've sent $${amount} to ${selectedContact.name}`,
        [
          { 
            text: 'OK', 
            onPress: () => router.back()
  ...
  return (
    ...
    >
      <Stack.Screen options={{ title: 'Send Money', headerShown: true }} /> //added headerShown
